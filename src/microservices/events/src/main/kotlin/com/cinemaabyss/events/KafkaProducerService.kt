...
object KafkaProducerService {
        ...
        fun sendEvent(topic: String, event: Event): RecordMetadata {
                val key = generateKeyFromPayload(event.payload)
                val value = objectMapper.writeValueAsString(event) // Сериализуем весь Event!

                val record = ProducerRecord(topic, key, value)
                val metadata =
                        producer.send(record).get() // .get() блокирует поток, но для MVP проще
        ...
        private fun generateKeyFromPayload(payload: EventPayload): String {
                return when (payload) {
